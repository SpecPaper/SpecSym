...
class Executor : public Interpreter {
  ...
  friend class RandomPathSPSearcher;
...
void transferToBasicBlockBTB(llvm::BasicBlock *src, 
        klee::KInstIterator &btbPC, 
        ExecutionState &state);
  ...
  /// speculative states should be evaluated before calls,
  /// if btb is empty state should be terminated.
  bool evaluateState(ExecutionState &state, 
                   KInstruction *ki,
                   llvm::Function *f);
  ...
  void executeMemoryOperation(ExecutionState &state,
                              ...
                              ref<Expr> value /* undef if read */,
                              KInstruction *target /* undef if write */,
                              bool& csm /* returns true if state.ctrlSpecMemOp is activated*/);
  ...
  StatePair fork(ExecutionState &current, ref<Expr> condition, bool isInternal, enum FunctionName fn);
  ...
  // call exit handler and terminate state: After Ret instruction
  ...
  /// Terminate a speculative state due to the SEW reduces to 0
  void terminateSpecState(ExecutionState &state);
  /// Terminate a speculative state due to BTB
  void terminateInfantSpecState(ExecutionState &state);
  ...
  void doDumpStates();
  void printAvailableStates(std::set<ExecutionState*> s);
