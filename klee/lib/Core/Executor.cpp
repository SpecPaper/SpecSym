...
namespace klee {
    ...
    cl::OptionCategory SpecCat("Speculative execution options", 
            "These options impact the specualtive paths exploring and the cache/BTB/PHT modeling");
...
namespace {
    ...
    cl::opt<bool>
        EnableSpeculativeExecution("enable-speculative",
                cl::init(false),
                cl::desc("Enable Speculative exeuction modeling (default=off)."),
                cl::cat(SpecCat));

    cl::opt<bool>
        EnableCacheModeling("enable-cachemodel",
                cl::init(false),
                cl::desc("Enable Cache modeling (default=off)."),
                cl::cat(SpecCat));

    cl::opt<bool>
        EnablePHTModeling("enable-phtmodel",
                cl::init(false),
                cl::desc("Enable PHT modeling (default=off)."),
                cl::cat(SpecCat));

    cl::opt<bool>
        EnableBTBModeling("enable-btbmodel",
                cl::init(false),
                cl::desc("Enable BTB modeling (default=off)."),
                cl::cat(SpecCat));

    cl::opt<unsigned> MaxSew(
            "max-sew",
            cl::desc(
                "Maximum SEW (default=10)"),
            cl::init(10),
            cl::cat(SpecCat));

    cl::opt<unsigned> CacheWays(
            "cache-ways",
            cl::desc(
                "Cache ways (default=2)"),
            cl::init(2),
            cl::cat(SpecCat));

    cl::opt<unsigned> CacheLineSize(
            "cache-line-size",
            cl::desc(
    ...
    cl::opt<unsigned> CacheSets(
            "cache-sets",
            cl::desc(
                "Cache sets (default=256)"),
            cl::init(256),
            cl::cat(SpecCat));
    ...
    /*
    -------------------------
    |       Reg Bits        |       -> table size = 2^(Reg Bits)
    -------------------------
    */
    cl::opt<unsigned> phtRegSize(
            "PHT-reg-size",
            cl::desc(
                "PHT register size (default=8)"),
            cl::init(8),
            cl::cat(SpecCat));
    ...
    */
    ...
    cl::opt<unsigned> BTBSets(
            "BTB-sets",
            cl::desc(
                "BTB sets (default=256)"),
            cl::init(256),
            cl::cat(SpecCat));

    cl::opt<unsigned> BTBTagBits(
            "BTB-tag-bits",
            cl::desc(
                "Number of BTB tag bits (default=20)"),
            cl::init(20),
            cl::cat(SpecCat));
    ...
    */ 
    ...
    //enable control spec load for the following example
    /*
    if (idx < array1_size) {                  
        if (array1[idx] == k)
            temp &= array2[0];
    ...
    */
    cl::opt<bool>
        ControlSpecMemoryOp("enable-CSMem",
                cl::init(false),
                cl::desc("Enable control-dependent speculative memory operation (default=off)."),
                cl::cat(SpecCat));

    /*cl::opt<std::string> InputPattern(
        "pattern", 
        cl::desc("Specify input pattern filename"), 
...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
    ...
    ivcEnabled(false), debugLogBuffer(debugBufferString), ispecEnabled (false) {


        /* SPSE */
        ispecEnabled = EnableSpeculativeExecution;
        isCacheEnabled = EnableCacheModeling;
        //editing
        isBTBEnabled = EnableBTBModeling;
        isPHTEnabled = EnablePHTModeling;
        isCSMOpEnabled = ControlSpecMemoryOp;

        if (ispecEnabled) {
            maxSEW = MaxSew;
            klee_warning("@Speculative execution modeling is enable! maxSEW=%d", maxSEW);
        }

        if (EnableCacheModeling) {
            CacheConfig::cache_ways = CacheWays;
            CacheConfig::cache_line_size = CacheLineSize;
            CacheConfig::cache_set_size = CacheSets;
            //CacheConfig::cache_tag_size = CacheTagBits;
        }

        //editing
        if (EnableBTBModeling){
            klee::BTBConfig::btb_ways = BTBWays;
            klee::BTBConfig::btb_set_size = BTBSets;
            klee::BTBConfig::btb_tag_size = BTBTagBits;
        }

        if(EnablePHTModeling){
            klee::PHTConfig::pht_reg_size = phtRegSize;
        }
...
MemoryObject * Executor::addExternalObject(ExecutionState &state, 
        ...
        bool isReadOnly) {
    //klee_message("add ExternalObject");
...
void Executor::branch(ExecutionState &state, 
    ...
    // if(ispecEnabled && state.pc->inst){
    //     for (unsigned i = 0; i < N; i++){
    //         ExecutionState *spState;
    //         ++stats::spStatesPL;
    //         if (state.isSpeculative) {
    //             spState = result[i]->specBranch(state.specInstCount);
    //         }else {
    //             spState = result[i]->specBranch(maxSEW);
    //         } 

    //         ExecutionState *perentState = state.pPerentState;

    //         if (state.isSpeculative) {
    //             if(isBTBEnabled || isPHTEnabled) ++stats::spStatesPL;
    //             result[i]->pSpecState = spState;

    //             spState->pPerentState = perentState;

    //             perentState->addSpeState(spState);
    //         } else {

    //             result[i]->pSpecState = spState;

    //             spState->pPerentState = result[i];

    //             result[i]->addSpeState(spState);
    //         }
    //         spState->pTag = spState->pPerentState->tag;
    //     }
    // }

/*
if(fn == FunctionName::IndirectCall && ispecEnabled){
if (current.isSpeculative) {
    spTrueState = trueState->specBranch(current.specInstCount);
}else {
...
/*
if the condition is true, we have only true state.
if the condition is false, we have only false state.
if both of them can be true, we have two execution states,
*/
Executor::StatePair 
Executor::fork(ExecutionState &current, ref<Expr> condition, bool isInternal, enum FunctionName fn) {
  Solver::Validity res;
  std::map< ExecutionState*, std::vector<SeedInfo> >::iterator it = 
  ...
  //
  bool isInUserCode = true;
  std::string file(current.prevPC->info->file);
  if (file.find("POSIX") != std::string::npos || 
      file.find("libc/") != std::string::npos || 
  ...
  if (res==Solver::True) {
    ...
    if (isInUserCode && !isInternal && ispecEnabled) {
        ...
        if (current.isSpeculative) {
            spTrueState = current.specBranch(current.specInstCount);
        }else {
    ...
    /*
    if(current.pSpecState){
        klee_message("\n============================= Create TRUE %ld,  with Spec State: %ld", current.tag, current.pSpecState->tag);
    }else{
        klee_message("\n============================= Create TRUE %ld", current.tag);
    }
    */
    ExecutionState *spTrueState = &current;
    if(fn == FunctionName::IndirectCall && ispecEnabled){
        ...
        if (current.isSpeculative) {
            ...
            spTrueState->pPerentState = perentState;
            //klee_message("\n============================= parent of %ld is %ld", spTrueState->tag, perentState->tag);
            perentState->addSpeState(spTrueState);
    ...
    if (isInUserCode && !isInternal && ispecEnabled) {
        ...
        if (current.isSpeculative) {
            spFalseState = current.specBranch(current.specInstCount);
        }else {
    ...
    /*
    if(current.pSpecState){
        klee_message("\n============================= Create FALSE %ld,  with Spec State: %ld", current.tag, current.pSpecState->tag);
    }else{
        klee_message("\n============================= Create FALSE %ld", current.tag);
    }
    */
    ExecutionState *spFalseState = &current;
    if(fn == FunctionName::IndirectCall && ispecEnabled){
        ...
        if (current.isSpeculative) {
            ...
            //klee_message("\n============================= parent of %ld is %ld", spFalseState->tag, perentState->tag);
	...
	if (isInUserCode && !isInternal && ispecEnabled) {
        ...
        if (current.isSpeculative) {
            spTrueState = trueState->specBranch(current.specInstCount);        
            spFalseState = falseState->specBranch(current.specInstCount);
        }else {
            spTrueState = trueState->specBranch(maxSEW);        
        ...
        if (current.isSpeculative) {
            ...
            // klee_message("\n============================= parent of %ld is %ld", falseState->tag, perentState->tag);
            // klee_message("\n============================= parent of %ld is %ld", spTrueState->tag, perentState->tag);
            // klee_message("\n============================= parent of %ld is %ld", spFalseState->tag, perentState->tag);
            perentState->addSpeState(spTrueState);
            perentState->addSpeState(spFalseState);
        ...
        if(fn == FunctionName::IndirectCall && ispecEnabled){
            ...
            if (current.isSpeculative) {
                ...
                spTrueState->pPerentState = perentState;
                falseState->pPerentState = perentState;
                // klee_message("\n============================= current is: %ld", current.tag);
                // klee_message("\n============================= parent is: %ld", spTrueState->pPerentState->tag);
                // klee_message("\n============================= parent of %ld is %ld", spTrueState->tag, perentState->tag);
                // klee_message("\n============================= parent of %ld is %ld", falseState->tag, perentState->tag);
                perentState->addSpeState(spTrueState);
    ...
    /*
    if(trueState){
        if(trueState->pSpecState){
            klee_message("\n============================= Create T %ld,  with Spec State: %ld", trueState->tag, trueState->pSpecState->tag);
        }else{
            klee_message("\n============================= Create T %ld", trueState->tag);
        }
    }
    if(falseState){
        if(falseState->pSpecState){
            klee_message("\n============================= Create F %ld,  with Spec State: %ld", falseState->tag, falseState->pSpecState->tag);
        }else{
            klee_message("\n============================= Create F %ld", falseState->tag);
        }
    }
    */
...
const Cell& Executor::eval(KInstruction *ki, unsigned index, 
    ...
    if (vnumber < 0) {
        ...
        //llvm::errs() << "eval result:" << sf.locals[index].value << "\n";
        return sf.locals[index];
    }
}
// state.stack.back().locals[ki->dest];
void Executor::bindLocal(KInstruction *target, ExecutionState &state, 
        ref<Expr> value) {

    //llvm::errs() << "bindLocal: " <<value<<"\n";
    //llvm::errs() <<"asm line: " << target->info->assemblyLine<<"\n";
   ...
   if (value.isNull()) {
      llvm::errs() <<"asm line: " << target->info->assemblyLine<<"\n";
        llvm::errs() <<*(target->inst) <<"\n\n";
        llvm::errs() <<"index: " <<target->dest<<"\n";
        assert(false);
    }
    */
    //klee_message("bindlocal result %ld: ", target->dest);
    //klee_message("bindlocal result refCount %d: ", value->refCount); 
    getDestCell(state, target).value = value;
    //getDestCell(state, target).value->dump();
}
...
void Executor::printDebugInstructions(ExecutionState &state) {
    ...
    if (!DebugPrintInstructions.isSet(STDERR_COMPACT) &&
            !DebugPrintInstructions.isSet(FILE_COMPACT)) {
        if (state.isSpeculative) {
            (*stream) << "S"<< state.tag<<"."<<state.specInstCount;
        } else {
            (*stream) << state.tag;
        }
...
void Executor::stepInstruction(ExecutionState &state) {
    ...
    if (statsTracker && !state.isSpeculative) {
    ...
    if (ispecEnabled && state.isSpeculative) {
        state.specInstCount--;
    }
}

bool Executor::evaluateState(ExecutionState &state, 
        KInstruction *ki,
        Function *f){
    KFunction *kf = kmodule->functionMap[f];

    KInstIterator btbPC = state._BTBState->read(ki->info->line);
    unsigned btbDst = 0;
    if(btbPC){
        btbDst = btbPC->info->assemblyLine;    
    }

    KInstIterator realPC = kf->instructions;
    unsigned realDst = realPC->info->assemblyLine;

    //if they match
    if((realDst == btbDst)){
        return true;
    //if btb is not empty and they don't match
    }else if(btbDst != 0){
        return true;
    //if btb is empty
    }else{
...
void Executor::executeCall(ExecutionState &state, 
    ...
    if (f && f->isDeclaration()) {
        ...
        //klee_message("Function instruction size: %ld",kf->numInstructions);

        state.pushFrame(state.prevPC, kf);

        if(isBTBEnabled && isIndirect){
            KInstIterator btbPC = state._BTBState->read(ki->info->line);
            unsigned btbDst = 0;
            if(btbPC){
                btbDst = btbPC->info->assemblyLine;    
            ...
            if (state.isSpeculative)
            {
                //if they match
                if((realDst == btbDst)){
                    state.pPerentState->pSpecState = 0;
                    klee_message("Is spec and they match");
                    terminateInfantSpecState(state);
                    --stats::spStatesPL;
                    return;
                //if btb is not empty and they don't match
                }else if(btbDst != 0){
                    klee_message("Is spec and they don't match and BTB is not empty");
                    state.pc = btbPC;
                    //++stats::spStatesPL;
                } else {
                    state.pPerentState->pSpecState = 0;
                    klee_message("Is spec and BTB is empty");
                    terminateInfantSpecState(state);
                    --stats::spStatesPL;
                    return;
                }
            }else{
                state._BTBState->update(ki->info->line, realPC, state);
                state.pc = realPC;
            }
        }else{
            state.pc = realPC;
...
/*
PC = destination instruction address.
We use it in executeInstruction() to update PC of each state. 
*/
void Executor::transferToBasicBlock(BasicBlock *dst, BasicBlock *src, 
    ...
    //klee_message("branch target: %d", state.pc->info->assemblyLine);
...
/*
PC = destination instruction address.
We use it in speculative mode to execute instructions in BTB.
Should not use this when BTB is empty
*/
void Executor::transferToBasicBlockBTB(BasicBlock *src, KInstIterator &btbPC, 
        ExecutionState &state) {
    state.pc = btbPC;
...
void dumpStateConstraint(ExecutionState &state) {
    llvm::errs() << "==== Path Constraints ==== \n";
...
/*
Used in run()
for each instruction, it does the corresponding actions, e.g., updating PC, creating states, updating cache, etc. 
*/
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
    if (state.isSpeculative) {
        ...
        if (state.specInstCount <= 0) {
            ...
            //klee_message("\n=============================terminate spec state tag: %ld", state.tag);
            terminateSpecState(state);
            return;
        }

        // Speculative execution does not go inside the libs. 
        std::string file(ki->info->file);
        if (file.find("POSIX") != std::string::npos || 
            file.find("libc/") != std::string::npos || 
            file.find("klee-uclibc") != std::string::npos) {
            terminateSpecState(state);
    ...
    //klee_message("\n============================= tag: %ld,  inst: %d", state.tag, ki->info->assemblyLine);
    
    Instruction *i = ki->inst;


    /*
    unsigned assemblyLine = state.pc->info->assemblyLine;
    unsigned line = state.pc->info->line;
    klee_warning("opcode = %s, @asline=%d, @line=%d", i->getOpcodeName(), assemblyLine, line);
    */

    /*
    klee_message("\n================================");
    if (state.isSpeculative) {
        klee_message("SSSSSSSSSSSS%ld,  inst: %d, parent: %ld", state.tag, ki->info->assemblyLine, state.pTag);
    } else {
    ...
    */
    ...
    switch (i->getOpcode()) {
        ...
        */
        case Instruction::Br: {
                                  ...
                                  struct updateInfo uinfo(state, solver, ki);
                                  uinfo.isSpec = state.isSpeculative ? 1 : 0;
                                  uinfo.isSensitive = 2;
                                    /*
                                    enum OtherOps Instruction;
                                    uint8_t isSpec;
                                    uint8_t isSensitive; //only for address.
                                    uint8_t isConst;
                                    state.monitor->update(state, solver, ki, Load, state.isSpeculative, base2.get()->getSensitive());
                                    */
                                  ...
                                  if (bi->isUnconditional()) {
                                    ...
                                    uinfo.isConst = isa<ConstantExpr>(cond) ? 1 : 0;
                                    
                                    Executor::StatePair branches = fork(state, cond, false, FunctionName::CondBranch);
                                    /*
                                    if(branches.first){
                                        if(branches.first->pSpecState){
                                            klee_message("\n============================= Create %ld,  with Spec State: %ld", branches.first->tag, branches.first->pSpecState->tag);
                                        }else{
                                            klee_message("\n============================= Create %ld", branches.first->tag);
                                        }
                                        //klee_message("============================= Constraint, tag: %ld ======================", branches.first->tag);
                                        //dumpStateConstraint(*(branches.first));
                                    }
                                    if(branches.second){
                                        if(branches.second->pSpecState){
                                            klee_message("\n============================= Create %ld,  with Spec State: %ld", branches.second->tag, branches.second->pSpecState->tag);
                                        }else{
                                            klee_message("\n============================= Create %ld", branches.second->tag);
                                        }
                                        //klee_message("============================= Constraint, tag: %ld ======================", branches.second->tag);
                                        //dumpStateConstraint(*(branches.second));
                                    }
                                    */
                                    if(cond.get()->getSensitive()){
                                        klee_message("\n============================= State: (%ld) Condition is Sensitive ===============================================", state.tag);
                                    }
                                      // NOTE: There is a hidden dependency here, markBranchVisited
                                      // requires that we still be in the context of the branch
                                      // instruction (it reuses its statistic id). Should be cleaned
                                      // up with convenient instruction specific data.
                                    if (ki->info->isInUserCode && statsTracker && state.stack.back().kf->trackCoverage)
                                          statsTracker->markBranchVisited(branches.first, branches.second);

                                    /*
                                    """ This is a version that works fine (without history bits) ... 
                                        Here we are using the right version of taken, not taken for going from source code to llvm IR to simulate how llvm IR is compiled to x86.
                                        If we go from x86 to llvm IR, we switch the definitions.
                                            -Taken: jumping over the if-block
                                            -Not taken: Jumping into the if-block
                                        if BTB is null, we jump into the if-block
                                        -----------C source code----    
                                        if (x == 0){x++;} y++; 
                                        -------LLVM IR--------------
                                        if cond, L1, L2
                                    ...
                                    */
                                    if(isPHTEnabled){
                                        if(ispecEnabled){
                                            bool dst = state._PHTState->read();
                                            //not taken
                                            if(branches.first && branches.second && branches.first->isSpeculative && branches.second->isSpeculative){
                                                ++stats::spStatesCommon;
                                            }
                                            if (branches.first){
                                                transferToBasicBlock(bi->getSuccessor(0), bi->getParent(), *branches.first);
                                                // if it's real, update
                                                if( !branches.first->isSpeculative ){
                                                    branches.first->_PHTState->update(1);
                                                }
                                                if (branches.first->pSpecState) {
                                                    branches.first->monitor->update(uinfo);
                                                    // if it's not taken
                                                    if(!dst){
                                                        //llvm::errs() << "(PHT) Currently we are in Real first: tag: " << branches.first->tag << "\n";
                                                        --stats::spStatesPL;
                                                        terminateInfantSpecState(*(branches.first->pSpecState));
                                                        branches.first->pSpecState = 0;
                                                    }else{
                                                        //llvm::errs() << "(PHT) Currently we are in Spec second and then Real first, tag: " << branches.first->tag << ", spec_tag: " << branches.first->pSpecState->tag << "\n";
                                                        branches.first->pSpecState->monitor->update(uinfo);
                                                        transferToBasicBlock(bi->getSuccessor(1), bi->getParent(), *(branches.first->pSpecState));
                                                        branches.first->pSpecState->monitor->setReturnState(branches.first);
                                                        ++stats::spStatesCommon;
                                                    }
                                                }
                                            }
                                            ...
                                            if (branches.second){
                                                ...
                                                // if it's real, update
                                                if( !branches.second->isSpeculative ){
                                                    branches.second->_PHTState->update(0);
                                                }
                                                if (branches.second->pSpecState) {
                                                    // if it's not taken
                                                    branches.second->monitor->update(uinfo);
                                                    if(!dst){
                                                        //llvm::errs() << "(PHT) Currently we are in Spec first and then Real second, tag: " << branches.second->tag << ", spec_tag: " << branches.second->pSpecState->tag << "\n";
                                                        branches.second->pSpecState->monitor->update(uinfo);
                                                        transferToBasicBlock(bi->getSuccessor(0), bi->getParent(), *(branches.second->pSpecState));
                                                        branches.second->pSpecState->monitor->setReturnState(branches.second);
                                                        ++stats::spStatesCommon;
                                                    }else{
                                                        //llvm::errs() << "(PHT) Currently we are in Real second: tag: " << branches.second->tag << "\n";
                                                        --stats::spStatesPL;
                                                        terminateInfantSpecState(*(branches.second->pSpecState));
                                                        branches.second->pSpecState = 0;
                                                    }
                                                }
                                            }
                                        } else {
                                    ...
                                    else if(isBTBEnabled){
                                        if(ispecEnabled){
                                            //if we jump to the "then" block or "else" block, increment. At the end, if it's >1, increment spStatesCommon
                                            uint8_t inPlaceJump = 0;
                                            if(branches.first){
                                                //reading btb to check the destination. btbDst is the assembly line of the destination.
                                                KInstIterator btbPC = branches.first->_BTBState->read(ki->info->line);
                                                unsigned btbDst = 0;
                                                if(btbPC){
                                                    btbDst = btbPC->info->assemblyLine;    
                                                }

                                                //extracting assembly line of bi->getSuccessor(0)
                                                KFunction *kf = branches.first->stack.back().kf;
                                                unsigned entry = kf->basicBlockEntry[bi->getSuccessor(0)];
                                                KInstIterator pc = &kf->instructions[entry];
                                                unsigned realDst = pc->info->assemblyLine;
            
                                                
                                                //klee_warning("@btbDst=%d, @RealDst=%d", btbDst, realDst);

                                                //assert(branches.first->pSpecState);
                                                if (branches.first->pSpecState){

                                                    //if they match or the entry is empty
                                                    if((btbDst == realDst) || (btbDst == 0)){
                                                        branches.first->monitor->update(uinfo);
                                                        // Debuging
                                                        //llvm::errs() << "Currently we are in Real first\n";
                                                        //llvm::errs() << "Currently we are in Real first, tag: " << branches.first->tag << "\n";

                                                        transferToBasicBlock(bi->getSuccessor(0), bi->getParent(), *branches.first);
                                                        terminateInfantSpecState(*(branches.first->pSpecState));
                                                        --stats::spStatesPL;
                                                        branches.first->pSpecState = 0;
                                                        //currently we commit directly now. It will be relevant for "history bits"
                                                        inPlaceJump++;
                                                        if( !branches.first->isSpeculative ){
                                                            KFunction *kf = state.stack.back().kf;
                                                            unsigned entry = kf->basicBlockEntry[bi->getSuccessor(0)];
                                                            KInstIterator pc = &kf->instructions[entry];
                                                            branches.first->_BTBState->update(ki->info->line, pc, *branches.first);
                                                        }

                                                        if(isCSMOpEnabled){
                                                            if(cond.get()->getSensitive()){
                                                                if(branches.first->isSpeculative){
                                                                    branches.first->ctrlSpecMemOp = true;
                                                                    klee_message("=============Control Spec Load Activated, tag: %d ==============", branches.first->tag);
                                                                }
                                                            }
                                                        }
                                                    }
                                                    //if they don't match and the entry is not empty
                                                    else {
                                                        branches.first->monitor->update(uinfo);
                                                        branches.first->pSpecState->monitor->update(uinfo);
                                                        // Debuging
                                                        //KLEE_DEBUG_WITH_TYPE("btb", llvm::errs() << "Currently we are in Spec second and then Real first\n";);
                                                        //llvm::errs() << "Currently we are in Spec second and then Real first, tag: " << branches.first->tag << ", spec_tag: " << branches.first->pSpecState->tag << "\n";

                                                        transferToBasicBlockBTB(bi->getParent(), btbPC, *(branches.first->pSpecState));
                                                        branches.first->pSpecState->monitor->setReturnState(branches.first);
                                                        //transferToBasicBlock(bi->getSuccessor(1), bi->getParent(), *(branches.first->pSpecState));
                                                        //branches.first->pSpecState = 0;
                                                        transferToBasicBlock(bi->getSuccessor(0), bi->getParent(), *branches.first);
                                                        ...
                                                        if(isCSMOpEnabled){
                                                            if(cond.get()->getSensitive()){
                                                                ...
                                                                branches.first->pSpecState->ctrlSpecMemOp = true;
                                                                klee_message("=============Control Spec Load Activated, tag: %d ==============", branches.first->pSpecState->tag);
                                                            }
                                                        }
                                                    }
                                                }
                                            ...
                                            if(branches.second){
                                                //reading btb to check the destination. btbDst is the assembly line of the destination.
                                                KInstIterator btbPC = branches.second->_BTBState->read(ki->info->line);
                                                unsigned btbDst = 0;
                                                if(btbPC){
                                                    btbDst = btbPC->info->assemblyLine;    
                                                }
                                                ...
                                                //extracting assembly line of bi->getSuccessor(0)
                                                KFunction *kf = branches.second->stack.back().kf;
                                                unsigned entry = kf->basicBlockEntry[bi->getSuccessor(1)];
                                                KInstIterator pc = &kf->instructions[entry];
                                                unsigned realDst = pc->info->assemblyLine;

                                                //klee_warning("@btbDst=%d, @RealDst=%d", btbDst, realDst);

                                                assert(branches.second->pSpecState);
                                                if (branches.second->pSpecState) {
                                                    //if they match
                                                    if(btbDst == realDst){
                                                        branches.second->monitor->update(uinfo);
                                                        ...
                                                        // Debuging
                                                        //KLEE_DEBUG_WITH_TYPE("btb", llvm::errs() << "Currently we are in Real Second\n";);
                                                        //llvm::errs() << "Currently we are in Real Second, tag: " << branches.second->tag << "\n";
                                                        ...
                                                        branches.second->pSpecState = 0;
                                                        inPlaceJump++;
                                                        //This would be relevant when we have history bits.
                                                        if(!branches.second->isSpeculative){
                                                            KFunction *kf = state.stack.back().kf;
                                                            unsigned entry = kf->basicBlockEntry[bi->getSuccessor(1)];
                                                            KInstIterator pc = &kf->instructions[entry];
                                                            branches.second->_BTBState->update(ki->info->line, pc, *branches.second);
                                                        }

                                                        if(isCSMOpEnabled){
                                                            if(cond.get()->getSensitive()){
                                                                if(branches.second->isSpeculative){
                                                                    branches.second->ctrlSpecMemOp = true;
                                                                    klee_message("=============Control Spec Load Activated, tag: %d ==============", branches.second->tag);
                                                                }
                                                            }
                                                        }
                                                    }
                                                    //if they don't match or if the entry is empty.
                                                    else {
                                                        branches.second->monitor->update(uinfo);
                                                        branches.second->pSpecState->monitor->update(uinfo);
                                                        // Debuging
                                                        //KLEE_DEBUG_WITH_TYPE("btb", llvm::errs() << "Currently we are in Spec first and then Real second\n";);
                                                        //llvm::errs() << "Currently we are in Spec first and then Real second, tag: " << branches.second->tag << ", spec_tag: " << branches.second->pSpecState->tag << "\n";

                                                        if(btbDst != 0){
                                                            transferToBasicBlockBTB(bi->getParent(), btbPC, *(branches.second->pSpecState));
                                                        }
                                                        else {
                                                            transferToBasicBlock(bi->getSuccessor(0), bi->getParent(), *(branches.second->pSpecState));
                                                            inPlaceJump++;
                                                        ...
                                                        if(isCSMOpEnabled){
                                                            if(cond.get()->getSensitive()){
                                                                ...
                                                                branches.second->pSpecState->ctrlSpecMemOp = true;
                                                                klee_message("=============Control Spec Load Activated, tag: %d ==============", branches.second->pSpecState->tag);                                                                    
                                                            }
                                                        }
                                                    }
                                                }
                                            ...
                                            if(branches.first && branches.second && (inPlaceJump > 0)){
                                                ++stats::spStatesCommon;
                                            }
                                        }
                                        else {
                                      ...
                                      if (ispecEnabled) {
                                          if (branches.first && branches.first->pSpecState) {
                                              transferToBasicBlock(bi->getSuccessor(1), bi->getParent(), *(branches.first->pSpecState));
                                              branches.first->pSpecState->monitor->setReturnState(branches.first);
                                              branches.first->pSpecState = 0;
                                          }

                                          if (branches.second && branches.second->pSpecState) {
                                              transferToBasicBlock(bi->getSuccessor(0), bi->getParent(), *(branches.second->pSpecState));
                                              branches.second->pSpecState->monitor->setReturnState(branches.second);
                                              branches.second->pSpecState = 0;
                                          }
        ...
        case Instruction::Switch: {
                                    ...
                                    //   llvm::errs() << "cond before:";
                                    //   cond->print(llvm::errs());
                                    //   llvm::errs() << "\n";
                                    //   dumpStateConstraint(state);
                                       cond = toUnique(state, cond);
                                    //   llvm::errs() << "cond after:";
                                    //   cond->print(llvm::errs());
                                    //   llvm::errs() << "\n";
        ...
        case Instruction::Call: {
                                    ...
                                    uinfo.isSpec = state.isSpeculative ? 1 : 0;
                                    uinfo.isSensitive = 2;
                                    //isConst = 1 -> directCall, 0 -> indirectCall
                                    uinfo.isConst = f ? 1 : 0;
                                    ...
                                    if (f) {
                                        ...
                                        /*
                                        if(ispecEnabled && state.pSpecState){
                                            ++stats::spStatesPL;
                                            klee_message("========================== executing direct call on %ld ", state.pSpecState->tag);
                                            executeCall(*(state.pSpecState), ki, f, arguments);
                                        }
                                        if(state.isSpeculative)
                                            ++stats::spStatesPL;
                                        */
                                        //klee_message("========================== executing direct call on %ld ", state.tag);
                                        executeCall(state, ki, f, arguments, false);
                                        state.monitor->update(uinfo);
                                        if(ispecEnabled && state.pSpecState){
                                            state.pSpecState->monitor->update(uinfo);
                                            state.pSpecState->monitor->setReturnState(&state);
                                        }
                                        // to be fixed. *(state.pSpecState) is not created. it should be created using specBranch()
                                    } else {
                                        ...
                                        // if(state.isSpeculative){
                                        //     bool ret = evaluateState(state, ki, f);
                                        //     if(!ret){
                                        //         terminateSpecState(state);
                                        //         return;
                                        //     }
                                        ...
                                        bool hasInvalid = false, first = true, term = false;
                                        ...
                                        do {
                                            ...
                                            assert(success && "FIXME: Unhandled solver failure");
                                            (void) success;
                                            StatePair res = fork(*free, EqExpr::create(v, value), true, FunctionName::IndirectCall);
                                            if (res.first) {
                                                ...
                                                if (legalFunctions.count(addr)) {
                                                    ...
                                                    if(ispecEnabled && res.first->pSpecState){
                                                        //ll on %ld ", res.first->pSpecState->tag);
                                                        executeCall(*res.first->pSpecState, ki, f, arguments, true);
                                                    }
                                                    //klee_message("========================== executing call on %ld ", res.first->tag);
                                                    executeCall(*res.first, ki, f, arguments, true);
                                                    res.first->monitor->update(uinfo);
                                                    if(ispecEnabled && res.first->pSpecState){
                                                        res.first->pSpecState->monitor->update(uinfo);
                                                        res.first->pSpecState->monitor->setReturnState(&state);
                                                    }
                                                } 
                                                else {
                                                    if (!hasInvalid) {
        ...
        case Instruction::PHI: {
                                   ...
                                   if (result.isNull() && state.isSpeculative) {
                                        terminateSpecState(state);
                                        return;
                                   }
        ...
        case Instruction::ICmp: {
                                    ...
                                    switch(ii->getPredicate()) {
                                        case ICmpInst::ICMP_EQ: {
                                                                    ...
                                                                    bindLocal(ki, state, result);
                                                                    //klee_message("\n=============================(ICMP: next inst) tag: %ld,  inst: %d", state.tag, state.pc->info->assemblyLine);
                                                                    break;
                                                                }
        ...
        case Instruction::Load: {
                                    ...
                                    ref<Expr> base2 = eval(ki, 0, state).value;
                                    //llvm::errs() << "Value: ";
                                    //base2->print(llvm::errs());
                                    //llvm::errs() << "\n";
                                    struct updateInfo uinfo(state, solver, ki);
                                    uinfo.isSpec = state.isSpeculative ? 1 : 0;
                                    uinfo.isSensitive = base2.get()->getSensitive() ? 1 : 0;
                                    uinfo.isConst = isa<ConstantExpr>(base2) ? 1 : 0;
                                    /*
                                    enum OtherOps Instruction;
                                    uint8_t isSpec;
                                    uint8_t isSensitive; //only for address.
        ...
        case Instruction::Store: {
                                    ...
                                    // ref<Expr> base2 = eval(ki, 1, state).value;
                                    // ref<Expr> value2 = eval(ki, 0, state).value;
                                    struct updateInfo uinfo(state, solver, ki);
                                    uinfo.isSpec = state.isSpeculative ? 1 : 0;
                                    uinfo.isSensitive = (value.get()->getSensitive() || base.get()->getSensitive()) ? 1 : 0;
                                    uinfo.isConst = isa<ConstantExpr>(base) ? 1 : 0;
                                    /*
                                    enum OtherOps Instruction;
                                    uint8_t isSpec;
                                    uint8_t isSensitive; //only for address.
        ...
        case Instruction::IntToPtr: {
                                       ...
                                       //if (arg.isNull()) {
                                       // unsigned number = ki->info->assemblyLine;                                       
                                       // llvm::errs() <<"asm line: " << ki->info->assemblyLine<<"\n";
                                       // llvm::errs() <<*(ki->inst) <<"\n\n";
                                       //   assert(false);
                                       //}
                                        //llvm::errs()<<number<<"\n\n";
    ...
    //klee_message("\n=============================next inst: %d", state.pc->info->assemblyLine);
}
...
void Executor::updateStates(ExecutionState *current) {
    ...
    for (std::vector<ExecutionState *>::iterator it = removedStates.begin(),
        ...
        //if (es->isSpeculative)
         //   klee_message("Remove state: S%d", es->tag);
        //else
        //    klee_message("Remove state: %d", es->tag);
        
        // TODO: possible has problem here. 
        if (!es->isSpeculative)
            processTree->remove(es->ptreeNode);
        //klee_message("Delete state: %ld", es->tag);
        delete es;
    }
    removedStates.clear();
...
void Executor::doDumpStates() {
    ...
    //spectreRecorder.dump(llvm::errs());
...
void Executor::printAvailableStates(std::set<ExecutionState*> s){
    std::set<ExecutionState*>::iterator it = s.begin();
    klee_message("====================================================Available States:==============================================");
    while (it != s.end())
    {
        klee_message("%ld", (*it)->tag);
        it++;
    }
}
...
void Executor::run(ExecutionState &initialState) {
    ...
    while (!states.empty() && !haltExecution) {
        //for (std::set<ExecutionState*>::iterator it=states.begin(); it!=states.end(); ++it) {
            //ExecutionState *es = *it;
            //if (es->isSpeculative)
            //    klee_message("States==: S%d", es->tag);
            //else
            //    klee_message("States==: %d", es->tag);
        //}
        //printAvailableStates(states);
        ExecutionState &state = searcher->selectState();
        //klee_message("Picked State is: %ld", state.tag);
        //if (state.isSpeculative)
        //    klee_message("State: S%d", state.tag);
        //else
        //    klee_message("State: %d", state.tag);
        //
        //klee_message("State: %d, p=%d", state.tag, state.pPerentState->tag);
        //if (!state.pSpecState){
        KInstruction *ki = state.pc;
        //if(ki || (states.size() < 2)){
            //klee_message("\n============================= tag: %ld", state.tag);
            
            //if(ki) klee_message("============================= inst: %u", ki->info->assemblyLine);
            uint64_t a = *theStatisticManager->getStatisticByName("SpstatesPL");
            //klee_message("spStatesPL=%ld", a);
            uint64_t b = *theStatisticManager->getStatisticByName("SpstatesCommon");
            //klee_message("spStatesCommon=%ld", b);
...
void Executor::terminateState(ExecutionState &state) {
    ...
    if (isCacheEnabled) {
        state.cacheState->verifyCacheSideChannel(solver);
    }

    //klee_message("Terminate a state: %ld", state.tag);
    ...
    //if the terminated state is not among addedStates, move it to removedStates.
    //if it is among added states, just remove the state from addedStates before adding it to the set of total states.
...
void Executor::terminateSpecState(ExecutionState &state) {
    assert(state.isSpeculative);
    //klee_debug("Terminate a speculative state: s%ld", state.tag);
    //klee_message("Terminate a speculative state: s%ld.%d", state.tag, state.specInstCount);
    //terminateState(state);
    //klee_message("Terminate Speculative state: %ld", state.tag);
    //
    //assert(false);
    long executedInst = maxSEW - state.specInstCount;
    stats::totalSpecInst += executedInst;
    ++stats::spStatesDone;

    ExecutionState *perentState = state.pPerentState;
    // if((perentState->specStates.size() == 0) || (perentState->selectSpState().tag != state.tag)){
    //     terminateInfantSpecState(state);
    //     perentState->removeHeadState(isCacheEnabled);
    //     return;
    // }

    //klee_message("\n=============================terminate spec state tag: %ld", state.tag);

    state.monitor->copyMonitor();

    assert(perentState->selectSpState().tag == state.tag);

    perentState->removeHeadState(isCacheEnabled);
}

void Executor::terminateInfantSpecState(ExecutionState &state){
    assert(state.isSpeculative);
    
    //klee_message("\n=============================terminate infant spec state tag: %ld", state.tag);

    ExecutionState *parentState = state.pPerentState;

    for (auto it = parentState->specStates.begin(); it != parentState->specStates.end(); ++it) {
        if(((*it) == &state) && ((*it)->tag) == state.tag){
            delete (*it);
            parentState->specStates.erase(it);
            return;
        }
    }
}
...
void Executor::terminateStateEarly(ExecutionState &state, 
    ...
    //klee_message("Terminate an early state: %ld", state.tag);
...
void Executor::executeAlloc(ExecutionState &state,
    ...
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
        ...
        StatePair fixedSize = fork(state, EqExpr::create(example, size), true, FunctionName::none);
...
void Executor::executeFree(ExecutionState &state,
    ...
    StatePair zeroPointer = fork(state, Expr::createIsZero(address), true, FunctionName::none);
...
void Executor::resolveExact(ExecutionState &state,
    ...
    for (ResolutionList::iterator it = rl.begin(), ie = rl.end(); 
        ...
        StatePair branches = fork(*unbound, inBounds, true, FunctionName::none);
...
ref<Expr> Executor::createSensitiveValue(Expr::Width type) {
    static int count = 0;
    //klee_message("create sensitive value named secret");
...
void Executor::executeMemoryOperation(ExecutionState &state,
    ...
    //if (state.isSpeculative && (address.isNull())) {
        // Exceptions in speculative execution, just terminate the state
    //  terminateSpecState(state);
    //    return;
    //}
    ...
    bool success;
    solver->setTimeout(coreSolverTimeout);
    //address->print(llvm::errs());
    //klee_message("address resolve");
    //dumpStateConstraint(state);
    if (!state.addressSpace.resolveOne(state, solver, address, op, success)) {
        //llvm::errs() << "resolveOne failure\n";
        address = toConstant(state, address, "resolveOne failure");
        success = state.addressSpace.resolveOne(cast<ConstantExpr>(address), op);
    }
    solver->setTimeout(time::Span());

    //klee_message("Success: %d", success);
    ...
    if (success) {
        ...
        */
        ...
        //llvm::errs() << check <<"\n";
        // std::string info;
        // mo->getAllocInfo(info);
        // llvm::errs() << info << "\n";
        // //dumpStateConstraint(state);
        // address->print(llvm::errs());
        // klee_message("Address");
        // offset->print(llvm::errs());
        // klee_message("Offset");
        // check->print(llvm::errs());
        // klee_message("Bounds");
        bool success = solver->mustBeTrue(state, check, inBounds);
        //klee_message("Success: %d,\t InBound: %d", success, inBounds);
        solver->setTimeout(time::Span());
        ...
        llvm::errs() <<"check: " <<check<<"\n";
        llvm::errs() <<"InBounds: "<<inBounds<<"\n";
        if (isWrite) {
            llvm::errs() <<"Mem op: Write Value: "<<value<<"\n";
        } else {
            llvm::errs() <<"Mem op: Load!\n";
        }
        llvm::errs() <<"Address, sensitive: " <<address.get()->getSensitive()<<"\n";
        llvm::errs() << "Constrain sensitive: " <<state.getSensitivity()<<"\n";
            ...
            if (isWrite) {
                ...
                if (state.isSpeculative) {
                    ...
                    if(str.find("marked_secret") != std::string::npos){
                        klee_message("============================= Spec sensitive (inBounds) ======================");
                        static unsigned id;
                        const Array *array =
                            arrayCache.CreateArray("SECRET" + llvm::utostr(++id),
                                    Expr::getMinBytesForWidth(result->getWidth()));
                        //ref<Expr> res = Expr::createTempRead(array, result->getWidth());
                        UpdateList ul(array, 0);
                        ref<Expr> res;// = ReadExpr::create(ul, ConstantExpr::create(0, Expr::Int32));
                        switch (result->getWidth())
                        {
                        case Expr::Bool: 
                            res = ZExtExpr::create(ReadExpr::create(ul, 
                                                                    ConstantExpr::create(0, Expr::Int32)),
                                                    Expr::Bool);
                            break;
                        case Expr::Int8: 
                            res =  ReadExpr::create(ul, 
                                                    ConstantExpr::create(0,Expr::Int32));
                            break;
                        case Expr::Int16: 
                            res =  ConcatExpr::create(ReadExpr::create(ul, 
                                                                    ConstantExpr::create(1,Expr::Int32)),
                                                    ReadExpr::create(ul, 
                                                                    ConstantExpr::create(0,Expr::Int32)));
                            break;
                        case Expr::Int32: 
                            res =  ConcatExpr::create4(ReadExpr::create(ul, 
                            ...
                            break;
                        case Expr::Int64: 
                            res =  ConcatExpr::create8(ReadExpr::create(ul, 
                                                                        ConstantExpr::create(7,Expr::Int32)),
                                                    ReadExpr::create(ul, 
                                                                        ConstantExpr::create(6,Expr::Int32)),
                                                    ReadExpr::create(ul, 
                                                                        ConstantExpr::create(5,Expr::Int32)),
                                                    ReadExpr::create(ul, 
                        ...
                        //state.addConstraint(eq);
                        //result = res;
                        Expr *e = dyn_cast<Expr>(res);
                        e->setSensitive();
                        //res->setSensitive();
                        assert(res->getSensitive());
                        //llvm::errs() << "Res refCount: " << e->refCount << "\n";
                        //llvm::errs() << "Res marked_secret: ";
                        //res->dump();
                        bindLocal(target, state, res);
                        return;
                    }
                }
                //llvm::errs() << "Result load: ";
                //result->dump();
    ...
    for (ResolutionList::iterator i = rl.begin(), ie = rl.end(); i != ie; ++i) {
        ...
        if (bound) {
            //klee_message("============================= Constraint mem, tag: %ld ======================", branches.first->tag);
            //dumpStateConstraint(*(branches.first));
            if (isWrite) {
                if (os->readOnly) {
        ...
        unbound = branches.second;
        if(unbound){
            //klee_message("============================= Constraint mem, tag: %ld ======================", branches.second->tag);
            //dumpStateConstraint(*(branches.second));
        }
...
void Executor::executeMemoryOperation(ExecutionState &state,
    ...
    if (success) {
        ...
        */
        ...
        if (inBounds) {
            ...
            if(state.ctrlSpecMemOp && isCSMOpEnabled){
                csm = true;
                klee_message("=============Control Spec Load: (BOUND)==============");
            }
            return;
        }
    } 
    ...
    for (ResolutionList::iterator i = rl.begin(), ie = rl.end(); i != ie; ++i) {
        ...
        if(!state.isSpeculative){
            ...
            // bound can be 0 on failure or overlapped 
            if (bound) {
                //klee_message("============================= Constraint mem first, tag: %ld ======================", branches.first->tag);
                //dumpStateConstraint(*(branches.first));
                if (isWrite) {
                    if (os->readOnly) {
                        terminateStateOnError(*bound, "memory error: object read only",
            ...
            unbound = branches.second;
            if(unbound){
                //klee_message("============================= Constraint mem second, tag: %ld ======================", branches.second->tag);
                //dumpStateConstraint(*(branches.second));
            }
            if (!unbound)
                break;
    ...
    if (state.isSpeculative && !isWrite) {
        if(state.ctrlSpecMemOp){
            csm = true;
            klee_message("=============Control Spec Load: (specLoad)==============");
        }
        //check greater than 1 byte
        if (bytes != 1) {
...
void Executor::runFunctionAsMain(Function *f,
    ...
    //klee_message("Initial function instruction size: %ld",kmodule->functionMap[f]->numInstructions);
